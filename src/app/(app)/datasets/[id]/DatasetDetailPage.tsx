...
export default function DatasetDetailPage({ datasetId }: { datasetId: string }) {
  ...
  const [librarianMsg, setLibrarianMsg] = useState<{ ok: boolean; text: string } | null>(null);
  const fileInputRef = useRef<HTMLInputElement>(null);

  async function handleLibrarianIngest() {
    if (!librarianPath.trim() || librarianBusy) return;
    setLibrarianBusy(true);
    setLibrarianMsg(null);
    try {
      const result = await librarianIngest(librarianPath.trim(), datasetId);
      if (result.status === "ok") {
        setLibrarianMsg({
          ok: true,
          text: `${result.classified_as} (conf ${result.classifier_confidence}) · ${result.chunks_dispatched} chunks · cognee=${result.engines?.cognee?.written ?? 0} fbrain=${result.engines?.fbrain?.written ?? 0}`,
        });
        setLibrarianPath("");
        try {
        ...
        setLibrarianMsg({ ok: false, text: result.error ?? "ingest failed" });
      }
    } catch (err: any) {
      setLibrarianMsg({ ok: false, text: String(err?.message ?? err) });
    } finally {
  ...
  async function handleUpload(newFiles: FileList | File[]) {
    ...
    try {
      ...
      console.error(is429 ? "Batch already running:" : "Upload failed:", err);
    } finally {
      setUploading(false);
    }
  }
  ...
  return (
    ...
    >
      ...
      {/* Librarian — Add by path / URL / DB */}
      <div style={{ background: "#fff", border: "1px solid #E4E4E7", borderRadius: 8, padding: 14, display: "flex", flexDirection: "column", gap: 8 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <span style={{ fontSize: 13, fontWeight: 600, color: "#18181B" }}>Add by path / URL</span>
          <span style={{ fontSize: 11, color: "#A1A1AA" }}>Librarian routes folders → fbrain, DBs → cognee</span>
        </div>
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <input
            type="text"
            value={librarianPath}
            onChange={(e) => setLibrarianPath(e.target.value)}
            onKeyDown={(e) => { if (e.key === "Enter") handleLibrarianIngest(); }}
            disabled={librarianBusy}
            placeholder="/Users/you/kb  ·  sqlite:///path/to.db  ·  https://…"
            style={{ flex: 1, height: 36, border: "1px solid #E4E4E7", borderRadius: 6, paddingInline: 12, fontSize: 13, color: "#18181B", background: librarianBusy ? "#F9FAFB" : "#fff", outline: "none", fontFamily: "inherit" }}
          />
          <button
            onClick={handleLibrarianIngest}
            disabled={!librarianPath.trim() || librarianBusy}
            className="cursor-pointer hover:bg-cognee-purple-hover"
            style={{ background: librarianBusy || !librarianPath.trim() ? "#A78BFA" : "#6510F4", color: "#fff", border: "none", borderRadius: 6, padding: "8px 16px", fontSize: 13, fontWeight: 500, whiteSpace: "nowrap", opacity: librarianBusy || !librarianPath.trim() ? 0.6 : 1 }}
          >
            {librarianBusy ? "Ingesting..." : "Ingest"}
          </button>
        </div>
        {librarianMsg && (
          <span style={{ fontSize: 12, color: librarianMsg.ok ? "#16A34A" : "#DC2626" }}>
            {librarianMsg.ok ? "✓ " : "✗ "}{librarianMsg.text}
          </span>
        )}
      </div>
