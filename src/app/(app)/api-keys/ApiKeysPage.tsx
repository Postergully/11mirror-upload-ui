...
export default function ApiKeysPage() {
  ...
  return (
    <div style={{ padding: 32, display: "flex", flexDirection: "column", gap: 24, fontFamily: '"Inter", system-ui, sans-serif' }}>
      ...
      {/* MCP Connection (11Mirror Gateway) */}
      <div style={{ background: "#fff", border: "1px solid #E4E4E7", borderRadius: 12, padding: "20px 24px", display: "flex", flexDirection: "column", gap: 16 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#6510F4" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="3"/><path d="M12 1v4M12 19v4M4.22 4.22l2.83 2.83M16.95 16.95l2.83 2.83M1 12h4M19 12h4M4.22 19.78l2.83-2.83M16.95 7.05l2.83-2.83"/></svg>
          <span style={{ fontSize: 14, fontWeight: 600, color: "#18181B" }}>MCP Connection</span>
          <span style={{ fontSize: 12, color: "#A1A1AA" }}>Connect AI agents (Claude Cowork, Claude Code, ChatGPT)</span>
        </div>

        <div style={{ display: "flex", gap: 16 }}>
          <div style={{ flex: 2, display: "flex", flexDirection: "column", gap: 6 }}>
            <span style={{ fontSize: 12, fontWeight: 500, color: "#71717A" }}>MCP URL</span>
            <div style={{ display: "flex", alignItems: "center", gap: 8, background: "#F4F4F5", borderRadius: 8, padding: "10px 14px" }}>
              <span style={{ fontSize: 13, color: "#18181B", fontFamily: '"Fira Code", monospace', flex: 1, wordBreak: "break-all" }}>{"https://huff-displease-marshy.ngrok-free.dev/mcp"}</span>
              <CopyBtn id="mcp-url" text="https://huff-displease-marshy.ngrok-free.dev/mcp" copiedField={copiedField} setCopiedField={setCopiedField} />
            </div>
          </div>

          <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 6 }}>
            <span style={{ fontSize: 12, fontWeight: 500, color: "#71717A" }}>Auth Header</span>
            <div style={{ display: "flex", alignItems: "center", gap: 8, background: "#F4F4F5", borderRadius: 8, padding: "10px 14px" }}>
              <span style={{ fontSize: 13, color: "#18181B", fontFamily: '"Fira Code", monospace', flex: 1 }}>Authorization: Bearer</span>
            </div>
          </div>
        </div>

        <div style={{ display: "flex", gap: 12 }}>
          <div style={{ flex: 1, background: "#18181B", borderRadius: 8, padding: "12px 16px", display: "flex", flexDirection: "column", gap: 8 }}>
            <span style={{ fontSize: 11, color: "#A1A1AA", fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.05em" }}>Claude Code</span>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{ fontSize: 12, color: "#E4E4E7", fontFamily: '"Fira Code", monospace', flex: 1 }}>claude mcp add 11mirror --url https://huff-displease-marshy.ngrok-free.dev/mcp</span>
              <CopyBtn id="mcp-claude" text="claude mcp add 11mirror --url https://huff-displease-marshy.ngrok-free.dev/mcp" copiedField={copiedField} setCopiedField={setCopiedField} light />
            </div>
          </div>
          <div style={{ flex: 1, background: "#18181B", borderRadius: 8, padding: "12px 16px", display: "flex", flexDirection: "column", gap: 8 }}>
            <span style={{ fontSize: 11, color: "#A1A1AA", fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.05em" }}>Cowork Plugin</span>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{ fontSize: 12, color: "#E4E4E7", fontFamily: '"Fira Code", monospace', flex: 1 }}>Customize → Add Marketplace → Postergully/11mirror-plugin</span>
              <CopyBtn id="mcp-plugin" text="Postergully/11mirror-plugin" copiedField={copiedField} setCopiedField={setCopiedField} light />
            </div>
          </div>
        </div>
      </div>
